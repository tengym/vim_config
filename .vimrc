...
"""" Command Line
set history=5000            " Keep a very long command-line history.
...
let g:BASH_AuthorName   = 'tengym'     
let g:BASH_AuthorRef    = 'tengym'                         
let g:BASH_Email        = 'tengym@gospell.com'            
let g:BASH_Company      = '---'    

"""" for doxygen
let g:DoxygenToolkit_authorName="tengym <tengym@gospell.com>" 
...
nmap <F5>, <ESC>:Dox<cr>
imap <F5>, <ESC>:Dox<cr>
nnoremap <F8> <ESC>:call Do_make('')<cr><CR>
nnoremap mm <ESC>:call Do_make('')<cr><CR>
nnoremap mc <ESC>:make clean; make<cr><CR>
...
"<leader> 代表"\"意思, <silent>代表不回显, <M>代表Alt键
"expand("<cword>")代表光标所在变量或函数

nnoremap <silent><leader><leader> <ESC>:tabedit ~/.cmd_myself.rc<CR>

func SetFunc()
    call append(line(".")+0,"/*************************************************************") 
    call append(line(".")+1, "** Function: ")
    call append(line(".")+2, "** Descripe: ") 
    call append(line(".")+3, "**    Input: ") 
    call append(line(".")+4, "**   Output: ")
...
nnoremap <silent>fk :vsp .<CR>
"分割窗口改变窗口的尺寸
nnoremap    w=  :resize +5<CR>	"垂直
...
let Tlist_Auto_Open_Window=1 
let Tlist_Process_File_Always=1 "实时更新tags
let Tlist_Auto_Open=1 
"let Tlist_GainFocus_On_ToggleOpen=1    "光标定位到taglist
let Tlist_Show_Menu=1
nnoremap <silent> <F12> :TlistToggle<CR>
autocmd vimenter * TlistToggle "自动打开
...
"nmap <C-A><C-F> :FirstExplorerWindow<cr>
...
let g:ctrlp_custom_ignore = {
    ...
    \ 'file': '\v\.(o|c\~|out|in.out|po.out|exe|so|dll|zip|tar|tar.gz|pyc)$',
    \ }
...
"autocmd BufWritePost *.cpp,*.c,*.h silent! :!cscope Rbq . --sort=yes --c++-kinds=+p --fields=+iaS --extra=+q
...
"字符高亮
nnoremap <silent>ml :nohl<CR>	"取消高亮

"行号设置
nnoremap <silent><F2> :set nu!<CR>

"文件保存
nnoremap <silent><C-s> <ESC>:wall<cr>
inoremap <silent><C-s> <ESC>:wall<cr>
inoremap <silent>ww <ESC>:wall<cr>

nnoremap <silent>qq :q!<CR>
nnoremap <silent>qa :qa!<CR>

"函数跳转
nnoremap <C-k> :cs find g <C-R>=expand("<cword>")<CR><CR> "函数跳转
nnoremap <C-j> <C-]> "函数跳转
nnoremap <C-B> <C-T> "函数返回
...
"快速切换到shell
nnoremap <silent>q <ESC>:sh<CR><CR>
...
nnoremap ,m :call SwitchSourceHead()<CR>

"快速拷贝文件至/home/tengym/tftp目录
nnoremap <silent>tp :!cp ./hiapp.elf /home/tengym/tftp/ -rf<CR>

"删除光标处至行尾
nnoremap <silent>dn <ESC>d$     
"复制整个单词
nnoremap <silent>fy <ESC>vey
"删除当前单词并替换另一单词
